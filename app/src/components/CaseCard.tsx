...
export default function CaseCard({ record, isSelected, onClick }: { record: MedicalRecord; isSelected: boolean; onClick: () => void }) {
  ...
  const getContent = (key: string) => {
    switch (key) {
      case 'pathology':
        return (
          <div className="space-y-1">
            <p><span className="text-[#802008] font-medium">病因：</span>{record.metadata.cause || '未填写'}</p>
            <p><span className="text-[#802008] font-medium">病所：</span>{record.metadata.locationOfDisease || '未填写'}</p>
            <p><span className="text-[#802008] font-medium">病机：</span>{record.metadata.mechanism || '未填写'}</p>
          </div>
        );
      case 'analysis':
      ...
      case 'diagnosis':
        return (
          <div className="space-y-1">
            <p><span className="text-[#802008] font-medium">证候：</span>{record.metadata.symptoms.join('、') || '未填写'}</p>
            <p><span className="text-[#802008] font-medium">中医诊断：</span>{record.metadata.tcmDiagnosis.join('、') || '未填写'}</p>
            <p><span className="text-[#802008] font-medium">西医诊断：</span>{record.metadata.wmDiagnosis.join('、') || '未填写'}</p>
  ...
  return (
    <div onClick={onClick} className={`relative border-b border-gray-100 cursor-pointer transition-all ${isSelected ? 'bg-white shadow-sm ring-1 ring-[#c94d4d]/50' : 'bg-[#fdfbf7] hover:bg-white'}`}>
      <div className="px-4 py-3">
        {/* 标题: xx治x某xx病案 */}
        <h3 className={`text-[15px] leading-snug mb-1 ${isSelected ? 'text-[#601005] font-semibold' : 'text-gray-800 font-medium'}`}>
          {formatTitle()}
        </h3>
          ...
          {tabs.map((tab) => (
            <button
              ...
              className={`flex items-center gap-1 px-2.5 py-1 text-xs rounded-md transition-colors whitespace-nowrap ${
                activeTab === tab.key
                  ? 'bg-[#fdf2f2] text-[#601005] font-medium border border-[#f5b5b5]/60'
