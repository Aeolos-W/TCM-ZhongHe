    ...
    <div className="flex flex-wrap items-center gap-1.5 p-2 bg-white border border-gray-200 rounded-lg focus-within:border-[#c94d4d] focus-within:ring-1 focus-within:ring-[#c94d4d]/20 transition-all min-h-[36px]"
      ...
      {tags.map((tag, i) => (
        <span key={`${tag}-${i}`} className="inline-flex items-center gap-0.5 px-2 py-0.5 text-xs bg-[#fdf2f2] text-[#601005] border border-[#f5b5b5]/60 rounded-md">
          {tag}<button onClick={(e) => { e.stopPropagation(); removeTag(i); }} className="text-[#c94d4d] hover:text-[#802008]"><X className="w-3 h-3" /></button>
        </span>
      ))}
        ...
        {editing ? (
          <div className="flex items-center gap-1">
            <input ref={inputRef} value={editValue} onChange={(e) => setEditValue(e.target.value)} onKeyDown={handleKey} onBlur={handleSave}
              className="flex-1 min-w-0 px-2 py-1 text-sm text-gray-800 bg-white border border-[#c94d4d] rounded focus:outline-none focus:ring-1 focus:ring-[#c94d4d]/30" />
          ...
          <div className="flex items-center gap-1">
            ...
            <button onClick={() => setEditing(true)} className="opacity-0 group-hover:opacity-100 p-0.5 text-gray-400 hover:text-[#802008] transition-all"><Pencil className="w-3 h-3" /></button>
          </div>
        )}
      </div>
    </div>
...
export default function MetadataGrid({ record, onUpdate }: { record: MedicalRecord; onUpdate: (r: MedicalRecord) => void }) {
  ...
  return (
    <div>
      <div className="mb-2">
        <div className="text-[11px] font-semibold text-[#802008] uppercase tracking-wide mb-1 px-1">病理三要素</div>
