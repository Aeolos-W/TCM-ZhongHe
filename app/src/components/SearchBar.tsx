  ...
  return (
    <div className="shrink-0 px-3 sm:px-4 py-2.5 sm:py-3 bg-white border-b border-gray-200">
      <div className="flex items-center gap-2">
        <div className="flex-1 flex items-center bg-gray-50 border border-gray-200 rounded-lg focus-within:border-amber-400 focus-within:ring-1 focus-within:ring-amber-400/20 transition-all">
          <div className="relative" ref={dropdownRef}>
            ...
            {showDropdown && (
              <div className="absolute top-full left-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg py-1 z-50 min-w-[100px]">
                {modes.map((mode) => <button key={mode.value} onClick={() => { setSearchMode(mode.value); setShowDropdown(false); }} className={`block w-full text-left px-3 py-1.5 text-xs hover:bg-amber-50 transition-colors ${searchMode === mode.value ? 'text-amber-700 bg-amber-50' : 'text-gray-600'}`}>{mode.label}</button>)}
              </div>
            )}
          </div>
          ...
          <button className="p-2 text-gray-400 hover:text-amber-600 transition-colors"><Search className="w-4 h-4" /></button>
        </div>
        <button className="hidden sm:flex items-center gap-1.5 px-3 py-2 text-xs font-medium text-amber-700 bg-amber-50 hover:bg-amber-100 border border-amber-200 rounded-lg transition-colors"><Sparkles className="w-3.5 h-3.5" />类案AI</button>
      ...
      {state.searchQuery && <div className="mt-1.5 sm:mt-2 text-xs text-gray-500">共找到 <span className="text-amber-700 font-medium">{state.records.length}</span> 条医案</div>}
    </div>
  );
}
